...
pub fn parse_request_line(line: &str) -> Result<Request, ParseError> {
    ...
    if parts.len() != 3 {
        return Err(ParseError::InvalidRequest(format!(
            "Invalid request line: {trimmed_line}"
        )));
    }
        ...
        Ok(Ok(_n)) => {
            if request_line.ends_with("\n") {
                ...
                Err(std::io::Error::new(
                    ...
                    format!(
                        "Error: Request length exceeds the {} bytes limit.",
...
async fn handle_connection(
    ...
    let response = if request.method == "GET" && request.path == "/slow" {
        debug!("--> Memulai proses lambat (5 detik)...");
        tokio::time::sleep(Duration::from_secs(5)).await;
        debug!("--> Proses lambat selesai.");
                    ...
                    match read_request_line(&mut socket, config).await {
                        Ok(Some(_)) => {
                            ...
                            if let Err(e) = socket.write_all(response.as_bytes()).await {
                                warn!(error = %e, client_addr = %addr, "Gagal mengirim 503 ke klien");
                            }
                        }
                        Ok(None) => {}
                        Err(e) if e.kind() == std::io::ErrorKind::InvalidData => {
                            ...
                            if let Err(e) = socket.write_all(response.as_bytes()).await {
                                warn!(error = %e, client_addr = %addr, "Gagal mengirim 413 ke klien");
                            }
                        }
                        Err(e) => warn!(error = %e, client_addr = %addr, "Gagal membaca karena kesalahan I/O"),
                    }
                });
            }
        }
